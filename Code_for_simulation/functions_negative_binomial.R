...
K=1
nrep=4
for(T in ((K-1)*nrep+1):(K*nrep)){
data=read.csv(paste('Simulated_data/nbinom-50/Data-',T,'.csv',sep=""))
Y=data$Y
...
model_or=glm.nb(Y~-1+X+factor(id_or),data=data2)
summary(model_or)
...
#random effect with quasipoisson 
model=glmer.nb(Y~X-1+(1|factor(id)),data=data)
betarqs=as.numeric(coef(model)$"factor(id)"[1,-1])
arqs=coef(model)$"factor(id)"[,1]
aMSE_rqs=sum((atrue-arqs)^2)/m
se.beta_rqs=summary(model)$coef[,2]


#fixed effect with quasipoisson 
fix=glm(Y~-1+X+factor(id),family=quasipoisson)
afqs=as.vector(coef(fix)[-(1:p)])
betafqs=coef(fix)[1:p]
aMSE_fqs=sum((atrue-afqs)^2)/m
se.beta_fqs=summary(fix)$coef[1:p,2]

###latent class ######
Group=c(2,3,4,5,6,7)
fit1=allvc(Y~X1+X2,random=~1|id,random.distribution="np",k=Group[1],
           family=poisson(link=log),data=data,verbose = F)
...
save_res=list(ngroup=ngroup,ngroup_np=ngroup_np,betaest=betaest,se.beta=se.beta,
              ...
              aMSE_fqs=aMSE_fqs,aMSE_rqs=aMSE_rqs,
              ...
              betarqs=betarqs,se.beta_rqs=se.beta_rqs,
              betafqs=betafqs,se.beta_fqs=se.beta_fqs)
save_name = paste('Results/nbinom-50/intermediate_results',T,'.Rdata',sep="")
