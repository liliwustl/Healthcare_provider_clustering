...
Fn_SCAD=function(lambda){
  ...
  repeat{
    ...
    pi=exp(a1)/(1+exp(a1))
    W=diag(pi*(1-pi))
    Yt=(Z%*%ahat+X%*%betahat)+solve(W)%*%(Y-pi)
    xx=solve(t(X)%*%W%*%X)
...
v=3
T=1
data=read.csv(paste('Simulated_data/logistic-100/Data-',T,'.csv',sep=""))
Y=data$Y
...
nxi=length(xi)
#####fused effects model####
fix=glm(Y~-1+X+factor(id),family=quasibinomial,data=data)
...
for(i in 1:nxi){
  ...
  phat=exp(Z%*%ascad[[i]]+X%*%betascad[[i]])/(1+exp(Z%*%ascad[[i]]+X%*%betascad[[i]]))
  b1[i]=-2*sum(Y*log(phat)+(1-Y)*log(1-phat))
  Cn=log(log(N+p))
...
model1=glm(Y~-1+X+factor(id1),family=quasibinomial,data=data1)
...
gk[[1]]=which(atrue==level[1])
gk[[2]]=which(atrue==level[2])
gk[[3]]=which(atrue==level[3])
...
model_or=glm(Y~-1+X+factor(id_or),family=binomial,data=data2)
summary(model_or)
para_or=as.numeric(coef(model_or))
se_or=as.numeric(summary(model_or)$coef[,2])
betaor=para_or[1:p]
aor[gk[[1]]]=para_or[p+1]
aor[gk[[2]]]=para_or[p+2]
aor[gk[[3]]]=para_or[p+3]
...
#random effect
model=glmer(Y~X-1+(1|factor(id)),family=binomial)
...
#fixed effect
fix=glm(Y~-1+X+factor(id),family=binomial)
afix=as.vector(coef(fix)[-(1:p)])
betafix=coef(fix)[1:p]
...
save_name = paste('Results/logistic-100/intermediate_results',T,'.Rdata',sep="")
